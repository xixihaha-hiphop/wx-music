...
const userColletion = db.collection('user')
Page({
  ...
  async insertData(){
    const res = await userColletion.add({
      data: {
        name: '张三',
        age: 18
      }
  ...
  // {
  //   "read": true,
  //   "delete": "get(`database.user.${doc._id}`).isManager == true"
  // }
  async getData(){
    const res = await userColletion.get()
    console.log(res)
  },
  async delData(){
    const res = await userColletion.where({
      _id: '3d23c0a05f1a4b3b0077a0ae53134487'
    }).remove()
    console.log(res)
  },
  async readData20(){
